...
# 2、深拷贝浅拷贝
    # 浅copy会copy父对象，不会copy子对象，而是引用子对象，而深copy完全copy父对象和子对象；
    # 浅copy和深copy本质区别，浅copy是增加一个指针，指向一个已经存在的内存，而深copy是增加一个新的指针并且开辟一个新的内存，新增加的指针指向新增加的内存；
    # 深copy不会出现释放同一个内存的异常，浅copy会出现。
    # 扩展：赋值-其实就是对象的引用，
