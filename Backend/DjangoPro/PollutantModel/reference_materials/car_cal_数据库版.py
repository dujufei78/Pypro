...
def retrieve_trace_db(dbc, car_sid):  # 从数据库导入，前处理函数
    trace_lngs_raw = []
    trace_lats_raw = []
    trace_speeds_raw = []
    trace_ts_raw = []
    ...
    results = dbc.getResults(dataSql, False)

    if len(results) > 120:  # 排除记录数过少的异常数据
        # 第一步，处理被折叠的轨迹，形成完整序列
        for item in results:
            cur_lng = float(item['lng'])
            cur_lat = float(item['lat'])
            cur_ts = int(item['timestamp'])
            cur_speed = int(item['speed'])
    ...
    for item in port_area:
        count_port += traces_df['lng'][(traces_df['lng'] >= item[0]) & (traces_df['lng'] <= item[1]) & (traces_df['lat'] >= item[2]) & (traces_df['lat'] <= item[3])].count()
    ts2 = int(time.time())
    info['ts'] = ts2 - ts1
...
def classify(df, dict_pollutant, emission_level, oil):  # 此函数旨在对车辆运行模式进行分类并且计算各类占比
    ...
    df['STP'] = (0.5629625 * df['speed'] + 0.001538425 * df['speed'] * df['speed'] * df['speed'] + load * df['speed'] * (df['accelerate'] + 9.8 * 0.0415)) / 10
    df['rkv'] = pd.cut(df['speed'], [0, 0.44704, 11.176, 22.352, 100], labels=['1', '2', '3', '4'], right=False, include_lowest=True)
    df['rkstp'] = pd.cut(df['STP'], [-90, 0, 3, 6, 9, 12, 18, 24, 30, 90], labels=['1', '2', '3', '4', '5', '6', '7', '8', '9'], right=False, include_lowest=True)
    ...
    # BC1,BC2
    df['vsp'] = df['speed'] * (1.1 * df['accelerate'] + 9.81 * 0.0415 + 0.132) + 0.000302 * df['speed'] * df['speed'] * df['speed']
    if load < 20:
        if emission_level <= 4:
            df['bc1_er'] = 206 + 0.458 * df['vsp'] + 2.9 * df['vsp'] * df['vsp'] - 0.010 * df['vsp'] * df['vsp'] * df['vsp']
            df['bc2'] = 0.521 * df['pm']
        else:
            df['bc1_er'] = 60.4 - 0.306 * df['vsp'] - 0.051 * df['vsp'] * df['vsp'] + 0.004 * df['vsp'] * df['vsp'] * df['vsp']
            df['bc2'] = 0.267 * df['pm']

    else:
        if emission_level <= 3:
            df['bc1_er'] = 28.3 + 5.20 * df['vsp'] + 0.194 * df['vsp'] * df['vsp'] - 0.019 * df['vsp'] * df['vsp'] * df['vsp']
            df['bc2'] = 0.588 * df['pm']
        elif emission_level == 4:
            df['bc1_er'] = 122 + 2.84 * df['vsp'] - 0.015 * df['vsp'] * df['vsp'] - 0.00361 * df['vsp'] * df['vsp'] * df['vsp']
            df['bc2'] = 0.588 * df['pm']
        else:
            df['bc1_er'] = 68.8 + 0.772 * df['vsp'] + 0.015 * df['vsp'] * df['vsp'] - 0.00163 * df['vsp'] * df['vsp'] * df['vsp']
            df['bc2'] = 0.352 * df['pm']
...
if len(inputArgs) > 1:
    if len(inputArgs) == 3:
        start_group_index = int(inputArgs[1])
        end_group_index = int(inputArgs[2])
        group_step = 10000
    elif len(inputArgs) == 4:
        start_group_index = int(inputArgs[1])
        end_group_index = int(inputArgs[2])
        group_step = int(inputArgs[3])
    else:
...
dict_oil = {'柴油': 'dsl', '汽油': 'gas', '天然气': 'oth', '电动': 'oth', 'LNG': 'oth', '空值': 'oth', '其他': 'oth', "双燃料(同时用两种)": 'oth', 'CNG': 'oth', '混合动力': 'oth'}
dict_emission = dict(国一=1, 国二=2, 国三=3, 国四=4, 国五=5, 国六=5)
dict_pollutant_raw = dict(codsl=co_dsl, hcdsl=hc_dsl, noxdsl=nox_dsl, pmdsl=pm_dsl, cogas=co_gas, hcgas=hc_gas, noxgas=nox_gas, \
    pmgas=pm_gas, cooth=co_oth, hcoth=hc_oth, noxoth=nox_oth, pmoth=pm_oth, co2dsl=co2_dsl, co2gas=co2_gas, co2oth=co2_oth)
...
for group_index in range(start_group_index, end_group_index+1):
    ...
    results = dbr.getResults(dataSql, False)
    for item in results:
        car_info[item['ID']] = {
            'load': item['engine_power'],
            'oil': item['fuel'],
            'oil_level': item['emission']
        }
    ...
    for car_sid in range(start_index,end_index):
        ...
        while not get_data_status:
            try:
                trace_lngs, trace_lats, trace_speeds, trace_ts, trace_weights, db_info = retrieve_trace_db(dbc, car_sid)
                get_data_status = True
            except Exception as e:
                print('Query data fail, try again...')
                pass
        if len(trace_lngs) == 0:
            error_message = db_info.get('error', '')
            print('sid: %d, error: %s...' % (car_sid, error_message))
            continue

        count_raw = db_info.get('count_raw', 0)
        count_new = db_info.get('count_new', 0)
        ...
        try:
                        ...
                        if not prev_cluster == -1:
                            if cluster_count > cluster_sample:
                                ...
                                if not prev_cluster in cluster_infos_filter:
                                    cluster_infos_filter[prev_cluster] = [cluster_infos[prev_cluster][0], cluster_infos[prev_cluster][1], False, False]
                                total_length = 0
                                total_time = 0
                                index_list = []
                        cluster_count = 0
            ...
            print('sid: %d, time: %d/%d seconds, raw/new: %d/%d, clusters: %d, error: %s' % (car_sid, (carEndTime - carStartTime), (tempTime - startTime), count_raw, count_new, num_clusters, error_message))
            continue
        ...
        if a and cal_status:  # 只在经过港区时才进行如下计算
            try:
                ...
                for i, temp_item in enumerate(traces_order):
                    if not temp_item[0] < 0:
                        if cluster_infos_filter[temp_item[0]][3]:
                            ...
                            service_label = 'P: '
                        if temp_item[0] != traces_order[i + 1][0] and service_label != 'Service: ':  # 把休息站的距离和时间叠加到下一个停留点
                            ...
                            if judge:
                                ...
                                if between_judge > 1:
                                    ...
                                    betw_port.append([record[pre_index].record[0], record[index].record[0], temp_length])
                                count += 1
                                index = cluster_infos_filter[temp_item[0]][4]
                                region_dict['总体'] = region_dict['总体'] + tmp_list + temp_item[4]
                                region_dict[record[index].record[0]] = region_dict[record[index].record[0]] + tmp_list + temp_item[4]
                                teu_dict[record[index].record[0]][0] = teu_dict[record[index].record[0]][0] + sin_teu # 进港teu计算
                                ...
                                if last_judge:
                                    ...
                                    region_dict['总体'] = region_dict['总体'] + tmp_list + temp_item[4]
                                    region_dict[record[index].record[0]] = region_dict[record[index].record[0]] + tmp_list + temp_item[4]
                                    teu_dict[record[index].record[0]][0] = teu_dict[record[index].record[0]][0] + sin_teu  # 出港teu计算
                ...
                # 累加网格数据
                for key in port_list.keys():
                    grid_pollution[key] = grid_pollution_cal(grid_pollution[key], region_dict[key], trace_lngs, trace_lats, pollution_list, pol_dict)
        ...
        if len(region_dict.get('总体',[])) == 0:
            ...
            total_car_port += 1
            print('sid: %d, time: %d/%d seconds, raw/new: %d/%d, clusters: %d, total_count: %d' % (car_sid, (carEndTime - carStartTime), (tempTime - startTime), count_raw, count_new, num_clusters, count))

        del trace_lngs, trace_lats, trace_speeds, trace_ts, db_info, traces, num_clusters, cluster_lables, dict_pollutant, cluster_infos, cluster_infos_filter, region_dict
        gc.collect()
        ...
        for pol in pollution_list:
            for lng_key in grid_pollution[key][pol].keys():
                for lat_key in grid_pollution[key][pol][lng_key].keys():
                    try:
                        temp_row = [key, pol, str(lng_key), str(lat_key), str(grid_pollution[key][pol][lng_key][lat_key])]
